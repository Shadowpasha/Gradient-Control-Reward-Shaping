...
class PendulumEnv(gym.Env):
    ...
    * v0: Initial versions release
    ...
    def __init__(self, render_mode: Optional[str] = None, g=10.0, enable_logging: bool = True):
        self.max_speed = 8
        self.max_torque = 2.0
        self.dt = 0.05
        self.g = g
        ...
        if self.enable_logging:
            self.writer = SummaryWriter("runs/upright_time_potential_based_" + datetime.now().strftime("%m_%d_%Y_%H_%M_%S"))
    ...
    def step(self, u):
        ...
        if(abs(angle_normalize(self.state[0])) < 0.0872  and abs(self.state[1]) < 0.1):
            # self.upright_times.append([time.time() - self.episode_start,self.step_counter] )
            # self.upright = True
            # truncated = True
